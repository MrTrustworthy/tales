...
class MapGrid(object):
    ...
    def mixed_heightmap(self):
        ...
        print
        "HM1:", mode1, hm1.max(), hm1.min(), hm1.mean()
        hm2 = self.single_heightmap(mode2)
        print
        "HM2:", mode2, hm2.max(), hm2.min(), hm2.mean()
        mix = 20 * (self.dvxs[:, 0] - self.dvxs[:, 1])
        mixing = 1 / (1 + np.exp(-mix))
        print
        "MIX:", mixing.max(), mixing.min(), mixing.mean()
    ...
    def shore_heightmap(self):
        ...
        print
        "Edge height:", self.elevation[:-1][self.edge].max()
    ...
    def island_heightmap(self):
        ...
        for i, u in enumerate(np.linspace(xs.min(), xs.max(), n) - 0.05):
            ...
            print
            "Erupting", self.vxs[np.argmax(eruption), :]
    ...
    def infill(self):
        ...
        while True:
            ...
            if tries == 1:
                print
                "Infilling", np.sum(sinks != -1), np.mean(self.vxs[sinks
                                                                   != -1, :], 0),
    ...
    def shortest_path(self, start, end, ):
        try:
            ...
            print
            "WARNING: Uncached path search", start, end
            pass
        best_dir = {}
        q = []
        flipped = False
    ...
    def plot(self, filename, rivers=True, cmap=mpl.cm.Greys, **kwargs):
        ...
        for terr in sorted(np.unique(self.territories),
            ...
            print
            "Labelling %s at %.1f" % (name, scores.max())
            reglabels.append(xy)
